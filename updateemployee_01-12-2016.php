				...
				if ($RS->num_rows > 0) 
				{
					...
					while($row = $RS->fetch_assoc())
					{
						...
						else
						{
							...
							if($FreeService!="0")
									{
										if($strSecondEmployees!="0" || $strSecondEmployees!="")
										{
											
												$sqlInsert = "INSERT INTO tblAppointmentAssignEmployee(AppointmentID, ServiceID, MECID, Commission,FreeService) VALUES
							('".$strAppointmentID."', '".$strServiceID."', '".$strSecondEmployees."', '".$strStatus."','1')";
							$DB->query($sqlInsert);
										}
							...
							//echo $sqlInsert;
									...
									else
									{
										...
										$sqlInsert = "INSERT INTO tblAppointmentAssignEmployee(AppointmentID, ServiceID, MECID, Commission) VALUES('".$strAppointmentID."', '".$strServiceID."', '".$strSecondEmployees."', '".$strStatus."')";
							            $DB->query($sqlInsert);
										}
									}
						...
						// Insert first employee
							if($FreeService!="0")
									{
										if($strFirstEmployees!="0" || $strFirstEmployees!="")
										{
											 
											$sqlInsert2 = "INSERT INTO tblAppointmentAssignEmployee(AppointmentID, ServiceID, MECID, Commission,FreeService) VALUES
						('".$strAppointmentID."', '".$strServiceID."', '".$strFirstEmployees."', '".$strStatus."','1')";
						$DB->query($sqlInsert2);
										}
									}
									else
...
$DB = Connect();
 $sepcontq=select("ServiceID","tblAppointmentsDetailsInvoice","AppointmentID='".$app_id."'");
        foreach($sepcontq as $vapp)
		{
			$serviee[]=$vapp['ServiceID'];
		}
		for($i=0;$i<count($serviee);$i++)
		{
			  $sepcont=select("MECID","tblAppointmentAssignEmployee","AppointmentID='".$app_id."' and ServiceID='".$serviee[$i]."'");
        foreach($sepcont as $vap)
		{
			$emp[]=$vap['MECID'];
		}
		}
