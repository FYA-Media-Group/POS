...
<?php
	...
	$strMyActionPage = "appointment_invoice2.php";
	$strMessage = "";
	$sqlColumn = "";
	$sqlColumnValues = "";
			...
			if(offervalue!="")
			{
				...
				//alert(result)
						 if($.trim(result)=='2')
						{
							...
							alert(msg)
							window.location="appointment_invoice2.php?uid="+app_id;
						}
...
/*   function checkstatus(ext)
		...
		if(ext=='Hold')
		{
			...
			window.location="appointment_invoice2.php?uid=<?php echo $appointment_id ?>";
		}
			  ...
			  //print_r($sericesd);
					...
					else
					{
															...
															if(in_array("$StoreIDd",$stores))
															    {
																	foreach($seldpdept as $val)
																		    {
																			$totalammt=0;
																			$serviceidd=$val['ServiceID'];
																	     	$AppointmentID=$val['AppointmentID'];
																			if(in_array("$serviceid",$servicessf))
																				{
																					$seldpdeptpp=select("*","tblAppointmentsDetails","AppointmentID='".$_GET['uid']."' and ServiceID='".$serviceid."'");
																					
																					foreach($seldpdeptpp as $valpp)
																					{
																						$seramt=$valpp['ServiceAmount'];
																						$qtyp=$valpp['qty'];
																						$amountp=$qtyp*$seramt;
					                                                                    $totalamountpp=$amountp*$Discount/100;
						...
						$DB->close();
                                            ...
                                            <td>
                                                ...
                                                <tbody>
                                                    ...
                                                    <tr>
                                                      ...
                                                      <td width="20%" style="LINE-HEIGHT:25px;FONT-FAMILY:Verdana,Geneva,sans-serif;COLOR:#000;FONT-SIZE:12px;FONT-WEIGHT:bold; text-align:center;"><input type="text" name="sub_total" readonly value="<?php echo number_format($sub_total,2);?>" />
													</td>
                                                    </tr>
												    <?php 
												 	$seldember=select("*","tblAppointments","AppointmentID='".$_GET['uid']."'");
																			     ...
																			     $totalpp=$totalpp+$totals;
																					if($baseamt!="")
																		            {
																						...
																						//echo 1;
																							...
																							$offeramount=$offeramount+$offeramt;
													 ...
													 if($offeramount!="")
													 {
														 ...
														 ?>
														  <td width="30%" style="LINE-HEIGHT:25px;FONT-FAMILY:Verdana,Geneva,sans-serif;COLOR:#000;FONT-SIZE:14px;FONT-WEIGHT:bold; text-align:right;">
																							Offer Discount</td>
																							 <td width="20%" style="LINE-HEIGHT:25px;FONT-FAMILY:Verdana,Geneva,sans-serif;COLOR:#000;FONT-SIZE:14px;FONT-WEIGHT:bold; text-align:center;">
																							<input id="offeramt" type="text" name="offeramt" readonly value="<?="-".$offeramount ?>" /></td>
														 <?php
													 }
													 ?>
										...
										$total=$total+$sub_total+$amountdetail-$offeramount;
													?>
                        ...
                        <tr>
                            <td>&nbsp;</td>
									...
									$seldppay=select("*","tblInvoiceDetails","AppointmentId='".$_GET['uid']."'");
										...
										$flag=$seldppay[0]['Flag'];
										?>
										<a href="appointment_invoice2.php" class="btn btn-primary" style="float: left;">Go Back</a>
...
<script>
 	function checkstatus(ext,ep)
		...
		if(ext=='Hold')
		{
			//alert(1234)
			// window.location="ManageAppointments.php";
			window.location="appointment_invoice2.php?uid="+ep;
		}
		else if(ext=='Completed')
		{
