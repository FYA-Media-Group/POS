...
<?php
	...
	$strMyActionPage = "ViewNonUsers.php";
	$strMessage = "";
	$sqlColumn = "";
	$sqlColumnValues = "";
							 	...
							 	if(typecomment!="")
								{
									...
									else if(typecomment=='2')
									{
										...
										$.ajax({
												type:"post",
												data:"customer="+customer+"&comment="+comment+"&app="+app+"&typecomment="+typecomment,
												url:"UpdateNonCustomerCallRemark1.php",
												success:function(result1)
												{
												if($.trim(result1)=='2')
												{
...
// Create connection And Write Values
  $DB = Connect();
										if($strStore!='0')
										{
										$Productst="Select * from tblCustomerRemarks WHERE StoreID='$strStore' and NonCommentType='".$typeremark."'";
										}
					                    ...
					                    if($RSaT->num_rows > 0) 
										{
											...
											while($rowa = $RSaT->fetch_assoc())
											{
												...
												$CommentType=$selptrtqistorettui[0]['NonCommentType'];
