			...
			$sql = "Select ServiceID from tblAppointmentsDetailsInvoice where AppointmentId='$strAppointmentID'";
				...
				if ($RS->num_rows > 0) 
				{
					...
					while($row = $RS->fetch_assoc())
					{
						...
						$strSecondEmployee = $_POST['Employee'];
						//print_r($strSecondEmployee);
						//$strSecondEmployee = explode("#SU#",$strSecondEmployee);
						
						//$strSecondEmployee = $strSecondEmployee[1];
						for($p=0;$p<count($strSecondEmployee);$p++)
						{
							
							if($strSecondEmployee[1]=="")
							{
								if($strSecondEmployee[$p]!="")
								{
								$strStatus = '1';
								$sqlInsert = "INSERT INTO tblAppointmentAssignEmployee(AppointmentID, ServiceID, MECID, Commission) VALUES
								('".$strAppointmentID."', '".$strServiceID."', '".$strSecondEmployee[$p]."', '".$strStatus."')";
								//echo $sqlInsert;
								$DB->query($sqlInsert);
								}
							}
...
?>	
			...
			<tr id="my_data_tr_<?=$counter?>">
				...
				<td>
					...
					<br>
						<select id="select<?=$strServiceID?>" name="Employee[]" class="form-control">
