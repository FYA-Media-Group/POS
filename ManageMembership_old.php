...
<?php
	$strPageTitle = "Manage Memberships | Nailspa";
	$strDisplayTitle = "Manage Memberships for Nailspa";
	$strMenuID = "10";
	$strMyTable = "tblMembership";
	$strMyTableID = "MembershipID";
	$strMyField = "MembershipName";
	$strMyActionPage = "ManageMemberships.php";
	$strMessage = "";
	$sqlColumn = "";
	$sqlColumnValues = "";
...
// code for not allowing the normal admin to access the super admin rights	
	...
	if ($_SERVER["REQUEST_METHOD"] == "POST")
	{
		...
		if($strStep=="add")
		{
			...
			{
				$sqlInsert = "INSERT INTO $strMyTable (MembershipName, Type, DiscountType, Discount, TimeForDiscountEnd, NotValidOnServices, Status,storeid) VALUES 
				('".$strMembershipName."', '".$strType."', '".$strDiscountType."', '".$strDiscount."', '".$strEndDay1."', '".$strNotValidOnServices."', '".$strStatus."','".$storename."')";
				ExecuteNQ($sqlInsert);
				echo $sqlInsert;
				// die();
								...
								<div class="example-box-wrapper">
									<div class="tabs">
										<ul>
											...
											<div class="panel-body">
												<h3 class="title-hero">List of Memberships | Nailspa</h3>
												<div class="example-box-wrapper">
...
if ($RS->num_rows > 0) 
{
	...
	{
		$counter ++;
		$strMembershipID = $row["MembershipID"];
		$getUID = EncodeQ($strMembershipID);
		$getUIDDelete = Encode($strMembershipID);		
		$MembershipName = $row["MembershipName"];
		$Type = $row["Type"];
		$DiscountType = $row["DiscountType"];
		$Discount = $row["Discount"];
		$TimeForDiscountEnd = $row["TimeForDiscountEnd"];
		
		$NotValidOnServices = $row["NotValidOnServices"];
		$arrNotValidOnServices = array();
		$arrNotValidOnServices = explode(",",$NotValidOnServices);
...
?>	
															<tr id="my_data_tr_<?=$counter?>">
																...
																<td style="text-align: center">
																	...
																	<a class="btn btn-link font-red" font-redhref="javascript:;" onclick="DeleteData('Step21','<?=$getUIDDelete?>', 'Are you sure you want to delete this Membership - <?=$AdminFullName?>?','my_data_tr_<?=$counter?>');">Delete</a><br>
																	<?php  
																	...
																	{
																		?>
																		<a class="btn btn-link" href="<?=$strMyActionPage?>?renew=<?=$strMembershipID?>">Renew</a>
																		<?php
																	}
																	?>
																</td>
...
?>	
													<div class="form-group"><label class="col-sm-3 control-label"><?=str_replace("MembershipName", "Membership Name", $row["Field"])?> <span>*</span></label>
															<div class="col-sm-3"><input type="text" name="<?=$row["Field"]?>" id="<?=str_replace("MembershipName", "Membership Name", $row["Field"])?>" class="form-control required" placeholder="<?=str_replace("MembershipName", "Membership Name", $row["Field"])?>"></div>
													</div>
<?php
		}
...
?>	
													<div class="form-group"><label class="col-sm-3 control-label"><?=str_replace("Discount", "Discount", $row["Field"])?> <span>*</span></label>
															<div class="col-sm-3"><input type="text" name="<?=$row["Field"]?>" id="<?=str_replace("Discount", "Discount", $row["Field"])?>" class="form-control required" placeholder="<?=str_replace("Discount", "Discount", $row["Field"])?>"></div>
													</div>
<?
		}
	/* 	else if ($row["Field"]=="TimeForDiscountEnd")
		{
?>	
													<div class="form-group"><label class="col-sm-3 control-label"><?=str_replace("TimeForDiscountEnd", "Discount Ends On", $row["Field"])?> <span>*</span></label>
															<!--<div class="col-sm-3"><input type="date" name="<?//=$row["Field"]?>[]" id="<?//=str_replace("TimeForDiscountEnd", "Discount Ends On", $row["Field"])?>" class="form-control required" placeholder="<?//=str_replace("TimeForDiscountEnd", "Discount Ends On", $row["Field"])?>"></div>
													</div>-->
													<div class="col-sm-3">
															<div class="input-prepend input-group"><span class="add-on input-group-addon"><i class="glyph-icon icon-calendar"></i></span> <input type="text" name="EndDay" id="EndDay"  class="form-control" data-date-format="YY-MM-DD" value="<?php echo date('Y-m-d');?>">
															</div>
														</div>
													</div>
													<!--<div class="form-group"><label class="col-sm-3 control-label"><?=str_replace("TimeForDiscountEnd", "Discount Ends At", $row["Field"])?> <span>*</span></label>
															<div class="col-sm-3"><input type="time" name="<?//=$row["Field"]?>[]" id="<?=str_replace("TimeForDiscountEnd", "Discount Ends At", $row["Field"])?>" class="form-control required" placeholder="<?//=str_replace("TimeForDiscountEnd", "Discount Ends At", $row["Field"])?>"></div>
													</div>-->
<?
		} */
...
?>											
													<div class="form-group"><label class="col-sm-3 control-label"><?=str_replace("NotValidOnServices", "Not Valid On Services", $row["Field"])?> <span>*</span></label><p>Press control and select</p>
														<div class="col-sm-4">
															<select name="<?=$row["Field"]?>[]" class="form-control" multiple>
...
?>	
											<div class="form-group"><label class="col-sm-3 control-label"><?=str_replace("TimeForDiscountEnd", "Discount Ends On", $key)?> <span>*</span></label>
												<div class="col-sm-3"><input type="date" name="<?=$key?>[]" class="form-control required" placeholder="<?=str_replace("TimeForDiscountEnd", "Discount Ends On", $key)?>" value="<?=$arrDateTime['0']?>" readonly></div>
											</div>
