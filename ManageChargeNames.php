...
<?php
	...
	$strMenuID = "10";
	$strMyTable = "tblChargeNames";
	$strMyTableID = "ChargeNameID";
	$strMyField = "ChargeName";
	$strMyActionPage = "ManageChargeNames.php";
	$strMessage = "";
	$sqlColumn = "";
	$sqlColumnValues = "";
...
// code for not allowing the normal admin to access the super admin rights	
	...
	if ($_SERVER["REQUEST_METHOD"] == "POST")
	{
		...
		if($strStep=="add")
		{
			...
			$strChargeName = Filter($_POST["ChargeName"]);
			...
			{
				$sqlInsert = "INSERT INTO $strMyTable (ChargeName, Status) VALUES ('".$strChargeName."', '".$strStatus."')";
				ExecuteNQ($sqlInsert);
				$DB->close();
            ...
            <div id="page-content-wrapper">
                <div id="page-content">
                    ...
                    <div id="page-title">
                        ...
                        <p>Add, Edit, Delete Charge Names</p>
                    </div>
<?php
								...
								<div class="example-box-wrapper">
									<div class="tabs">
										<ul>
											...
											<div class="panel-body">
												<h3 class="title-hero">List of Charge Names | Nailspa</h3>
												<div class="example-box-wrapper">
...
$DB = Connect();
$sql = "SELECT ChargeNameID, ChargeName, Status FROM tblChargeNames WHERE Status='0'";
$RS = $DB->query($sql);
if ($RS->num_rows > 0) 
{
	$counter = 0;
	...
	{
		$counter ++;
		$strChargeNameID = $row["ChargeNameID"];
		$getUID = EncodeQ($strChargeNameID);
		$getUIDDelete = Encode($strChargeNameID);		
															...
															<tr id="my_data_tr_<?=$counter?>">
																...
																<td style="text-align: center">
																	...
																	if($strAdminRoleID=="36")
                                                                    {
																		...
																		?>
																	<a class="btn btn-link font-red" font-redhref="javascript:;" onclick="DeleteData('Step12','<?=$getUIDDelete?>', 'Are you sure you want to delete this Charge - <?=$AdminFullName?>?','my_data_tr_<?=$counter?>');">Delete</a>
																	<?php 
																	}
																	?>
																</td>
...
?>
										...
										<div id="normal-tabs-2">
											...
											<input type="hidden" name="step" value="add">
												...
												<h3 class="title-hero">Add Charges</h3>
												<div class="example-box-wrapper">
...
if ($RS->num_rows > 0) 
{
	...
	while($row = $RS->fetch_assoc())
	{
		...
		else if ($row["Field"]=="ChargeName")
		{
?>	
													<div class="form-group"><label class="col-sm-3 control-label"><?=str_replace("ChargeName", "Charge Name", $row["Field"])?> <span>*</span></label>
															<div class="col-sm-3"><input type="text" name="<?=$row["Field"]?>" id="<?=str_replace("ChargeName", "Charge Name", $row["Field"])?>" class="form-control required" placeholder="<?=str_replace("ChargeName", "Charge Name", $row["Field"])?>"></div>
													</div>
<?php
		}
...
<?php
			...
			elseif($key=="ChargeName")
			{
?>	
											<div class="form-group"><label class="col-sm-3 control-label"><?=str_replace("ChargeName", "Charge Name", $key)?> <span>*</span></label>
												<div class="col-sm-3"><input type="text" name="<?=$key?>" class="form-control required" placeholder="<?=str_replace("ChargeName", "Charge Name", $key)?>" value="<?=$row[$key]?>"></div>
											</div>
<?php
			}
			elseif($key=="Status")
