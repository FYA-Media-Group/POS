...
<?php require_once 'incFirewall.fya'; 
	...
	$strMyActionPage = "CancelOrders.php";
	$strMessage = "";
	$sqlColumn = "";
	$sqlColumnValues = "";
...
// code for not allowing the normal admin to access the super admin rights	
	...
	$id=$_POST['id'];
			...
			{
				$sqlUpdate = "UPDATE tblFinalOrder SET Status='8' where OrderID='".$orderidd."' and Status!='2' and ID='".$id[$i]."'";
					ExecuteNQ($sqlUpdate);
		 ...
		 $seldatap=select("*","tblFinalOrder","OrderID='".$orderidd."' and Status='5' and StoredID='".$strStore."'");
...
$DB = Connect();
	...
	?>
				...
				<div id="normal-tabs-2">
					<div class="fa-hover">	
								<a class="btn btn-primary btn-lg btn-block" href="CancelOrders.php"><i class="fa fa-backward"></i> &nbsp; Go back to <?=$strPageTitle?></a>
							</div>
					...
					<div class="fa-hover">	
								<a class="btn btn-primary btn-lg btn-block" href="CancelOrders.php"><i class="fa fa-backward"></i> &nbsp; Go back to <?=$strPageTitle?></a>
											...
											<input type="hidden" name="step" value="add">
												...
												<h3 class="title-hero">Cancel Orders </h3>  
...
// Create connection And Write Values
$DB = Connect(); 
$sql = "Select * FROM tblFinalOrder where Status='5' and StoredID='".$strStore."'";
$RS = $DB->query($sql);
if ($RS->num_rows > 0) 
{
	$counter = 0;
...
?>	
												...
												<td><?=$counter?></td>
													...
													<td><?=$ProductStock?></td>
													<td><?=$OrderStock?></td>
													<td><?=$status?>  / <a href="IssueOrders.php?uid=<?=$getUID?>" class="btn btn-link">Reopen Order</a></td>
