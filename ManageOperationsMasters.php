...
<?php
	$strPageTitle = "Manage Operations | Nailspa";
	$strDisplayTitle = "Manage Operations for Nailspa";
	$strMenuID = "11";
	$strMyTable = "tblOperationsMasters";
	$strMyTableID = "OMID";
	$strMyField = "OperationName";
	$strMyActionPage = "ManageOperationsMasters.php";
	$strMessage = "";
	$sqlColumn = "";
	$sqlColumnValues = "";
...
// code for not allowing the normal admin to access the super admin rights	
	...
	if ($_SERVER["REQUEST_METHOD"] == "POST")
	{
		...
		if($strStep=="add")
		{
			...
			$strOperationName = Filter($_POST["OperationName"]);
			...
			if ($RS->num_rows > 0) 
			{
				...
				die('<div class="alert alert-close alert-danger">
					...
					<div class="alert-content">
						...
						<p>This Operation already exists in our system. Please try again with a different name.</p>
					</div>
				</div>');
			}
			else
			{
				$sqlInsert = "INSERT INTO $strMyTable (OperationName, Status) VALUES ('".$strOperationName."', '".$strStatus."')";
				ExecuteNQ($sqlInsert);
				$DB->close();
								...
								<div class="example-box-wrapper">
									<div class="tabs">
										<ul>
											...
											<div class="panel-body">
												<h3 class="title-hero">List of Operations | Nailspa</h3>
												<div class="example-box-wrapper">
...
if ($RS->num_rows > 0) 
{
	...
	while($row = $RS->fetch_assoc())
	{
		$counter ++;
		$strOMID = $row["OMID"];
		$getUID = EncodeQ($strOMID);
		$getUIDDelete = Encode($strOMID);		
...
?>	
															<tr id="my_data_tr_<?=$counter?>">
																...
																<td>
																	...
																	<a class="btn btn-link font-red" font-redhref="javascript:;" onclick="DeleteData('Step15','<?=$getUIDDelete?>', 'Are you sure you want to delete this Operation - <?=$AdminFullName?>?','my_data_tr_<?=$counter?>');">Delete</a>
																</td>
															</tr>
<?php
	}
...
if ($RS->num_rows > 0) 
{
	...
	while($row = $RS->fetch_assoc())
	{
		...
		else if ($row["Field"]=="OperationName")
		{
?>	
													<div class="form-group"><label class="col-sm-3 control-label"><?=str_replace("OperationName", "Operation Name", $row["Field"])?> <span>*</span></label>
															<div class="col-sm-3"><input type="text" name="<?=$row["Field"]?>" id="<?=str_replace("OperationName", "Operation Name", $row["Field"])?>" class="form-control required" placeholder="<?=str_replace("OperationName", "Operation Name", $row["Field"])?>"></div>
													</div>
<?php
		}
						...
						<div class="panel-body">
							...
							<form role="form" class="form-horizontal bordered-row enquiry_form" onSubmit="proceed_formsubmit('.enquiry_form', '<?=$strMyActionPage?>', '.result_message', '', '.admin_email', '.admin_password'); return false;">
								...
								<input type="hidden" name="step" value="edit">
									...
									<h3 class="title-hero">Edit Operations</h3>
									<div class="example-box-wrapper">
...
<?php
			...
			elseif($key=="OperationName")
			{
?>	
											<div class="form-group"><label class="col-sm-3 control-label"><?=str_replace("OperationName", "Operation Name", $key)?> <span>*</span></label>
												<div class="col-sm-3"><input type="text" name="<?=$key?>" class="form-control required" placeholder="<?=str_replace("OperationName", "Operation Name", $key)?>" value="<?=$row[$key]?>"></div>
											</div>
<?php
			}
			elseif($key=="Status")
