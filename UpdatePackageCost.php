...
<?php
	$strPageTitle = "Update Package Cost| Nailspa";
	$strDisplayTitle = "Update Package Cost for Nailspa";
	$strMenuID = "10";
	$strMyTable = "tblPackages";
	$strMyTableID = "PackageID";
	$strMyField = "PackageID";
	$strMyActionPage = "UpdatePackageCost.php";
	$strMessage = "";
	$sqlColumn = "";
	$sqlColumnValues = "";
 	...
 	if(packageid!="")
		{
			$.ajax({
				type:"post",
				data:"packageid="+packageid+"&stock="+stock,
				url:"UpdatedatePackageCost.php",
...
if(isset($_GET['uid']))
{
	...
	$Code=DecodeQ($_GET['uid']);

	?>
			<div class="fa-hover">	
								<a class="btn btn-primary btn-lg btn-block" href="ManagePackages.php"><i class="fa fa-backward"></i> &nbsp; Go back to <?=$strPageTitle?></a>
							</div>
                    <div class="panel">
						<div class="panel">
							<div class="panel-body">
								...
								<span class="form_result">&nbsp; <br></span>
									<div class="panel-body">
										<h3 class="title-hero">Update Cost For Package | Nailspa</h3>
										<div class="example-box-wrapper">
...
$DB = Connect();
$selp=select("*","tblPackages","Code='".$Code."'");
foreach($selp as $val)
{
	$counter ++;
	...
	$StoreID=$val['StoreID'];
	$Name=$val['Name'];
	$PackageNewPrice=$val['PackageNewPrice'];
	$code=$val['code'];
	...
	?>
			<tr id="my_data_tr_<?=$counter?>">
														<td><?=$counter?></td>
														<td><?=$StoreName?></td>
														<td><input type="hidden" id="PackageID" name="PackageID" value="<?=$PackageID?>"/><?=$Name?></td>
														<td contenteditable='true' id="PackageNewPrice"><?=$PackageNewPrice?></td>
